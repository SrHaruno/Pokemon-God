...
#===============================================================================
# Generates QR code bitmaps in byte mode, supporting versions 2-4.
#   Encodes arbitrary strings into a scannable QR code `Bitmap`.
class QRCodeGenerator
  # Class for automatic QR code version configuration.
  #   Holds version-dependent sizing, codeword and alignment data.
  class Config
    ...
    # @return [Integer] QR code version number
    attr_reader :version
    # @return [Symbol] error correction level (:L, :M, :Q, :H)
    attr_reader :error_level

    # Stores the version and error correction level for the configuration.
    ...
    # Calculates the QR matrix size for the configured version.
    # @return [Integer] number of modules per matrix side
    def size
      @size ||= 21 + 4 * (version - 1)
    end

    # Looks up the data codeword capacity for the configured version.
    # @return [Integer, NilClass] number of data codewords, nil for unsupported versions
    def data_codewords
      @data_codewords ||=
        { 2 => 34, 3 => 55, 4 => 80 }[version]
    end

    # Looks up the error correction codeword count for the configured version.
    # @return [Integer, NilClass] number of error correction codewords, nil for unsupported versions
    def ec_codewords
      @ec_codewords ||=
        { 2 => 10, 3 => 15, 4 => 20 }[version]
    end

    # Looks up the alignment pattern center positions for the configured version.
    # @return [Array<Integer>, NilClass] alignment pattern coordinates, nil for unsupported versions
    def alignment
      @alignment ||=
  ...
  class ReedSolomon
    ...
    class GF256
      # @return [Array<Integer>] exponent lookup table
      attr_reader :exp
      # @return [Array<Integer>] logarithm lookup table
      attr_reader :log
      ...
      # @param a [Array<Integer>] first polynomial coefficients
      # @param b [Array<Integer>] second polynomial coefficients
      # @return [Array<Integer>] product polynomial coefficients
      def mult(a, b)
      ...
      # Divides polynomial
      # @param dividend [Array<Integer>] dividend polynomial coefficients
      # @param divisor [Array<Integer>] divisor polynomial coefficients
      # @return [Array<Integer>] remainder of the polynomial division
      ...
      # @param x [Integer] first factor
      # @param y [Integer] second factor
      # @return [Integer] product within GF(256)
    ...
    # Constructs the GF256 table
    # @param config [QRCodeGenerator::Config] QR code configuration
    # @return [void]
    ...
    # @param data [Array<Integer>] data in bytes
    # @return [Array<Integer>] error correction codewords
    def ec(data)
    ...
    # @return [QRCodeGenerator::ReedSolomon::GF256] Galois field used for calculations
    attr_reader :gf256
    # @return [QRCodeGenerator::Config] QR code configuration
    attr_reader :config

    # Builds the Reed-Solomon generator polynomial for the configured codeword count.
  ...
  class Encoder
    ...
    # Stores the input data and configuration for encoding.
    ...
    # Encodes the data and error correction bytes as a single bit string.
    # @return [String] binary string of data and error correction bits
    def encode
    ...
    # @return [String] input data to encode
    attr_reader :data
    # @return [QRCodeGenerator::Config] QR code configuration
    attr_reader :config

    # Returns the input data in encoded form (bytes)
    # @return [Array<Integer>] encoded and padded data bytes
    def encoded_data
    ...
    # Calculates the maximum data bit capacity for the configured version.
    # @return [Integer] maximum number of data bits
    def max_bits
      @max_bits ||= config.data_codewords * 8
    end

    # Splits a bit string into 8-bit byte values.
    # @param bits [String] binary string to split
    # @return [Array<Integer>] byte values
  ...
  class Matrix
    ...
    # @return [Array<Array<Boolean, NilClass>>] 2D module matrix
    attr_reader :matrix
    # @return [QRCodeGenerator::Config] QR code configuration
    attr_reader :config
    ...
    # Returns the matrix size from the configuration.
    # @return [Integer] number of modules per matrix side
    def size
      config.size
    end
    ...
    # @param row [Integer] matrix row index
    # @param col [Integer] matrix column index
    # @return [Boolean] true if position overlaps a finder pattern area
    ...
    # @param x [Integer] left column of the pattern
    # @param y [Integer] top row of the pattern
    # @return [void]
    ...
    # Places a single alignment pattern centered on the given position.
    # @param center_x [Integer] center column of the pattern
    # @param center_y [Integer] center row of the pattern
    # @return [void]
  ...
  class << self
    ...
    # @param mode [Symbol] encoding mode (:numeric, :alphanumeric, :byte)
    # @return [Integer, NilClass] recommended version, nil if data too large
    ...
    # @param version [Integer] QR code version
    # @param mode [Symbol] encoding mode (:numeric, :alphanumeric, :byte)
    # @return [Integer, NilClass] approximate data capacity, nil for unsupported versions
  ...
  # Encodes the given data and prepares the QR code matrix for rendering.
  # @param data [String] data to encode into the QR code
  # @param version [Integer, NilClass] QR code version used, auto-selected when nil
  ...
  # @return [Bitmap] rendered QR code bitmap
  def generate
  ...
  # @return [QRCodeGenerator::Config] QR code configuration
  attr_reader :config
  # @return [QRCodeGenerator::Matrix] QR code module matrix
  attr_reader :matrix
  # @return [Integer] data unit width/height in pixels
  attr_reader :unit_size
  # @return [Color] color for "empty" QR code data
  attr_reader :color_white
  # @return [Color] color for "full" QR code data
  attr_reader :color_black

  # Select best suited QR version for input data
  # @param data [String] data to encode
  # @return [Integer] selected QR code version
  ...
  # @param bits [String] binary string of encoded data bits
  # @return [void]
  def map_bits(bits)
    col = matrix.size - 1
    dir_up = true
  ...
  # Checks whether a matrix position is reserved for function patterns.
  # @param row [Integer] matrix row index
  # @param col [Integer] matrix column index
  # @return [Boolean] true if the position cannot hold data bits
  ...
  # Checks whether a matrix position is reserved for format information.
  # @param row [Integer] matrix row index
  # @param col [Integer] matrix column index
  # @return [Boolean] true if the position holds format information
  ...
  # Checks whether a matrix position falls within an alignment pattern.
  # @param row [Integer] matrix row index
  # @param col [Integer] matrix column index
  # @return [Boolean] true if the position is inside an alignment pattern
  ...
  # Calculates and places the BCH-encoded format information bits in the matrix.
  # @return [void]
  ...
  # Calculates the output bitmap size including the quiet zone.
  # @return [Integer] bitmap width/height in pixels
  def bitmap_size
